...
pub struct PrintRequest {
    /// File name including extension, e.g. `bill-1024.pdf`. Used as the
    /// print job's display name in the system print dialog / queue.
    pub file_name: String,
    /// PDF file contents, base64 encoded (JSON can't carry raw bytes).
    pub base64: String,
}
...
pub struct PrintResponse {
    /// Always true on success — the command errors instead of returning
    /// false, since "the dialog didn't open" and "no printing support" are
    /// both surfaced as invoke rejections that `describePrintError` on the
    /// TS side turns into a readable reason.
    pub printed: bool,
}
...
impl<R: Runtime> AndroidSave<R> {
    ...
    pub fn print_pdf(&self, payload: PrintRequest) -> Result<PrintResponse> {
...
impl<R: Runtime> AndroidSave<R> {
    ...
    pub fn print_pdf(&self, _payload: PrintRequest) -> Result<PrintResponse> {
        Err(Error::Plugin(
            "android-save is only available on Android".into(),
        ))
    }
...
#[tauri::command]
fn print_pdf<R: Runtime>(app: tauri::AppHandle<R>, payload: PrintRequest) -> Result<PrintResponse> {
