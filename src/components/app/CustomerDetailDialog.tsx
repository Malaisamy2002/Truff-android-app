...
import { matchesCustomer, useBills, useUpdateBill } from "@/lib/data";
import {
  useSnackSales,
  useTurfBookings,
...
export function CustomerDetailDialog({ name, phone, onOpenChange }: Props) {
   ...
   * `spent - dues.total` rather than a separately-tracked figure — exactly
   * right whenever every unpaid item is captured by `dues` (true for every
   * current due type). */
   ...
   * just triggerable from here so the operator doesn't have to leave. A
   * manual single-line collect (from `PendingLineRow` below) shows its own
   * "Print receipt" toast action; `settleAll` passes `silent: true` and
   * fires one combined receipt for the whole balance instead, so a
   * multi-line settle doesn't pop one toast per booking/bill. */
  const collectLine = (
    line: DueLine,
    pay: number,
   ...
   * row context menu (Windows) and this dialog (both platforms) trigger the
   * identical action — this used to be its own inline copy of the same
   * math, which is exactly the kind of per-platform drift risk the shared
   * domain layer is meant to close off. */
  const settleAll = () => {
    if (!data) return;
    settleCustomer({
      name: name ?? "",
